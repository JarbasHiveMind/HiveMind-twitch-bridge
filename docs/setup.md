...
From nothing to a working Twitch chat bot backed by a HiveMind hub.
...
A chat message containing a trigger tag becomes a `recognizer_loop:utterance` sent to the hub. The sender's username travels in the message context, so the hub's `speak` reply is echoed back to the channel addressed to that user.
...
Edit the call to `connect_twitch_to_hivemind(...)` at the bottom of `twitch_bridge/__main__.py` with your `channel`, `oauth`, trigger `tags`, and HiveMind `host`/`port`/`key`. Then:
